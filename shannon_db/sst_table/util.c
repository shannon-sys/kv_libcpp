...
int little_endian = 1;  // intel: little by default
...
int get_varint32(slice_t *input, uint32_t *value)
{
	char *p = input->data;
...
int get_varint64(slice_t *input, uint64_t *value)
{
	char *p = input->data;
	char *limit = p + input->size;
...
{
	return (uint8_t*)(
			(unsigned long long)(pointer) + (N - 1)
...
void get_filesize(char *filename, size_t *file_size)
{
	...
	if (ret == 0) {
		*file_size = statbuf.st_size;
		...
		DEBUG("get file size failed: %s\n", strerror(errno));
		*file_size = 0;
	}
}

size_t read_file(char *filename, uint64_t offset, uint64_t size, slice_t *result)
{
	int fd;
	char *buf;
	ssize_t tmp_read;
	...
	if (size <= 0) {
		DEBUG("read size must bigger than 0\n");
		return 0;
	}
	...
	if (buf == NULL) {
		DEBUG("read file fail, malloc error\n");
		return 0;
	}

	if ((fd = open(filename, O_RDONLY)) == -1) {
		DEBUG("can not open file:%s, %s\n", filename, strerror(errno));
		goto free_buf;
	}
	if (lseek(fd, (off_t)(offset), SEEK_SET) == -1) {
		DEBUG("file=%s seek to offset=%lu failed, %s\n", filename, offset, strerror(errno));
		goto close_fd;
	}
	while (read_size < size) {
		tmp_read = read(fd, buf + read_size, size);
		if (tmp_read == -1 || tmp_read == 0) {
			DEBUG("file=%s read failed, %s\n", filename, strerror(errno));
			goto close_fd;
		}
		read_size += tmp_read;
		size -= tmp_read;
	...
	close(fd);
	set_slice(result, buf, read_size);
	return read_size;
close_fd:
	if (fd > 0)
		close(fd);
...
#if defined(_MSC_VER)
#  define xxhash_rotl32(x,r) _rotl(x,r)
#else
#  define xxhash_rotl32(x,r) ((x << r) | (x >> (32 - r)))
#endif

static inline uint32_t xxhash_swap32 (uint32_t x) {
...
#define XXHASH_PRIME32_1   2654435761U
#define XXHASH_PRIME32_2   2246822519U
#define XXHASH_PRIME32_3   3266489917U
#define XXHASH_PRIME32_4    668265263U
#define XXHASH_PRIME32_5    374761393U

static inline uint32_t xxhash_read_le32_align(const uint32_t *ptr, int align)
{
	if (!align)
		return little_endian ? A32(ptr) : xxhash_swap32(A32(ptr));
	else
		return little_endian ? *ptr : xxhash_swap32(*ptr);
}

static inline uint32_t xxhash_read_le32(const uint32_t *ptr)
{
	return xxhash_read_le32_align(ptr, 0);
}

// simple hash functions
static inline uint32_t xxhash32_align(const void *input, int len, uint32_t seed, int align)
{
	...
	if (len >= 16)
	{
		...
		uint32_t v1 = seed + XXHASH_PRIME32_1 + XXHASH_PRIME32_2;
		uint32_t v2 = seed + XXHASH_PRIME32_2;
		uint32_t v3 = seed + 0;
		uint32_t v4 = seed - XXHASH_PRIME32_1;

		do {
			v1 += xxhash_read_le32_align((const uint32_t *)p, align) * XXHASH_PRIME32_2;
			v1 = xxhash_rotl32(v1, 13); v1 *= XXHASH_PRIME32_1; p+=4;
			v2 += xxhash_read_le32_align((const uint32_t *)p, align) * XXHASH_PRIME32_2;
			v2 = xxhash_rotl32(v2, 13); v2 *= XXHASH_PRIME32_1; p+=4;
			v3 += xxhash_read_le32_align((const uint32_t *)p, align) * XXHASH_PRIME32_2;
			v3 = xxhash_rotl32(v3, 13); v3 *= XXHASH_PRIME32_1; p+=4;
			v4 += xxhash_read_le32_align((const uint32_t *)p, align) * XXHASH_PRIME32_2;
			v4 = xxhash_rotl32(v4, 13); v4 *= XXHASH_PRIME32_1; p+=4;
		} while (p<=limit);

		h32 = xxhash_rotl32(v1, 1) + xxhash_rotl32(v2, 7) + xxhash_rotl32(v3, 12) + xxhash_rotl32(v4, 18);
	} else {
		h32 = seed + XXHASH_PRIME32_5;
	}

	h32 += (uint32_t)len;

	while (p <= bend-4) {
		h32 += xxhash_read_le32_align((const uint32_t*)p, align) * XXHASH_PRIME32_3;
		h32  = xxhash_rotl32(h32, 17) * XXHASH_PRIME32_4;
		p+=4;
	}

	while (p < bend) {
		h32 += (*p) * XXHASH_PRIME32_5;
		h32 = xxhash_rotl32(h32, 11) * XXHASH_PRIME32_1;
		p++;
	}
...
uint32_t xxhash32(char *data, int len, uint32_t seed) {
	int align = 0;
#if !defined(XXH_USE_UNALIGNED_ACCESS)
	if (3 & (size_t)(data)) // input is aligned, let's leverage the speed advantage
		align = 1;
#endif
	return xxhash32_align(data, len, seed, align);
}
...
struct slice *new_slice(char *str, size_t n)
{
	...
	if (sli == NULL) {
		DEBUG("malloc sli failed,\n");
		return NULL;
	}
	sli->data = malloc(n);
	if (sli->data == NULL) {
		DEBUG("malloc sli->data failed,\n");
		return NULL;
	}
...
void delete_slice(struct slice *sli)
{
	assert(sli);
	if (sli->data)
		free(sli->data);
...
uint64_t pack_sequence_type(uint64_t seq, int type)
{
	assert(seq <= kMaxSequenceNumber);
	return (seq << 8) | type;
}

struct internalkey_t *internal_key(char *str, int n, uint64_t seq, int type)
{
	struct internalkey_t *key = malloc(sizeof(struct internalkey_t));
	if (key == NULL) {
		DEBUG("malloc key failed,\n");
		return NULL;
	}
	key->data = malloc(n);
	if (key->data == NULL) {
		DEBUG("malloc key->data failed,\n");
		return NULL;
	}
	memcpy(key->data, str, n);
	key->sequence = seq;
	key->type = type;
	key->size = n + 8;
	return key;
...
uint64_t extract_seq(struct slice *internal_key)
{
	assert(internal_key->size >= 8);
...
uint32_t extract_type(struct slice *internal_key)
{
	assert(internal_key->size >= 8);
	const size_t n = internal_key->size;
	uint64_t num = decode_fixed64(internal_key->data + n - 8);
...
char *append_key(char *key, int keylen, uint64_t time, int type)
{
	assert(key);
	assert(keylen >= 0);
	char *buf = malloc(keylen + 8);
	if (buf == NULL) {
		DEBUG("malloc buf failed,\n");
		return NULL;
	}
	memcpy(buf, key, keylen);
	put_fixed64(buf + keylen, pack_sequence_type(time, type));
	return buf;
}

char *make_name(char *name, uint64_t number, char *suffix)
{
	char buf[100];
	char *new_name = (char *) malloc(strlen(name) + sizeof (buf));
	snprintf(buf, sizeof(buf), "%06lu.%s", (unsigned long)(number), suffix);
	strcpy(new_name, name);
	strcat(new_name, buf);
	return new_name;
}

char *make_table_filename(char *name, char *cfname, uint64_t number, int is_rocksdb)
{
	char dir_name[256];
	strcpy(dir_name, "/tmp/");
	if (is_rocksdb) {
		strcat (dir_name, "rocksdb/");
		strcat (dir_name, name);
		strcat (dir_name, "/");
		create_dir(dir_name);
		strcat (dir_name, cfname);
		return make_name(dir_name, number, "sst");
	} else {
		strcat (dir_name, "leveldb/");
		strcat (dir_name, name);
		strcat (dir_name, "/");
		create_dir(dir_name);
		strcat (dir_name, cfname);
		return make_name(dir_name, number, "ldb");
	}
}
...
char *compression_type_to_string(int type)
{
	switch (type) {
	...
	case KSNAPPY_COMPRESSION:
		return "Snappy";
	case KZLIB_COMPRESSION:
		return "Zlib";
	case KBZIP2_COMPRESSION:
		return "BZip2";
	default:
		assert(false);
		return "";
...
int compress_block(slice_t *output, slice_t *input, int type)
{
	uLong uncomprLen = (uLong)input->size;
	Byte *uncompr;
	size_t ptr_size = 0;
	char *ptr;
	int ret = KOK;
	...
	switch (type) {
	case KNO_COMPRESSION:
		set_slice(output, input->data, input->size);
		break;
	case KSNAPPY_COMPRESSION:
		ptr_size = snappy_max_compressed_length(input->size);
		ptr = (char *)malloc(ptr_size);
		ret = ((snappy_compress(input->data, input->size, ptr, &ptr_size) ==  SNAPPY_OK) && (good_compression_ratio(ptr_size, input->size)));
		if (ret) {
			set_slice(output, ptr, ptr_size);
			ret = KOK;
			ret = KOK;
		} else {
		...
		break;
	default:
		DEBUG("unknow compression type=%d\n", type);
		break;
	}
	return ret;
}
...
char *find_key_between_start_limit(char *start_data, int start_size,  char *limit_data, int limit_size)
{
	size_t min_length = (start_size < limit_size) ? (start_size) : (limit_size);
	size_t diff_index = 0;
	char *buf = NULL;
	...
	if (diff_index >= min_length) {
		diff_byte = (uint8_t)((limit_data)[diff_index]);
		buf = malloc(start_size + 1);
		if (buf ==  NULL) {
			DEBUG("malloc buff failed.\n");
			return NULL;
		}
		memcpy(buf, start_data, start_size);
		buf[start_size] = '1';
	} else {
		diff_byte = (uint8_t)((start_data)[diff_index]);
		if (diff_byte < (uint8_t)(0xff) && diff_byte + 1 < (uint8_t)(limit_data[diff_index])) {
			buf = malloc(start_size);
			if (buf ==  NULL) {
				DEBUG("malloc buff failed.\n");
				return NULL;
			}
			memcpy(buf, start_data, start_size);
			buf[start_size - 1] = diff_byte + 1;
		} else {
			buf = malloc(start_size + 1);
			if (buf ==  NULL) {
				DEBUG("malloc buff failed.\n");
